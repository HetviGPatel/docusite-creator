...
const items = ["CNC machines & lathes", "Fabrication & welding equipment", "Material handling cranes & hoists", "Conveyor systems & belts", "Automation & robotics equipment", "Compressors & pneumatic tools", "Pumps & valves", "Industrial safety equipment"];
    ...
    <>
      <section className="bg-hero-gradient py-20">
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-navy-foreground mt-2">Industrial Machinery</h1>
          <p className="mt-2 text-navy-foreground/70">300K+ products</p>
        </div>
      </section>
      <section className="py-20 bg-background">
        <div className="max-w-4xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {items.map(s => (<div key={s} className="flex items-start gap-3 bg-card rounded-xl p-5 border border-border shadow-card"><Factory className="w-5 h-5 text-gold mt-0.5 shrink-0" /><span className="text-muted-foreground">{s}</span></div>))}
          </div>
